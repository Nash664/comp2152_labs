...
if not input_invalid:
  ...
  @        ..@#% @     @&*#@(         % 
   &   (  @    (   / /   *    @  .   /  
    ...
    if "3 stars" in last_line:
        m_combat_strength +=1
        print("monster combat strenth increased by 1")

    elif "Monster has killed the hero previously" in last_line:
        combat_strength += 1
        print("heros combat strenth increased by 1")
...
while valid:
    ...
    # Check if input is not empty, is a number, and is between 0 and 3
    if num_dream_lvls.isdigit() and 0 <= int(num_dream_lvls) <= 3:
        num_dream_lvls = int(num_dream_lvls)  # Convert to integer
        if num_dream_lvls != 0:
            health_points -= 1  # Deduct health points if dream levels are more than 0
        crazy_level = functions_lab06.inception_dream(num_dream_lvls)  # Call the function
        combat_strength += crazy_level
        print("combat strength: " + str(combat_strength))
        print("health points: " + str(health_points))
        valid = False  # Exit loop if input is valid
    else:
...
if(health_points>0  and m_health_points==0):
    with open("lab06.txt", "a") as file:
        file.write("Hero " + short_name + " has killed a monster and gained " + str(num_stars) + " stars\n" ) 
elif(m_health_points>0 and health_points==0):
    with open("lab06.txt", "a") as file:
